    ...
    while(1){
        ...
        switch(result){
            ...
            case -1:
                perror("select");
                exit(1);
            // If, during the wait, you have some action on the file descriptor, read the input \
                on 'stdin' and echo it whenever an <end of line> character is received, until \
                that input is Ctrl+D.
            default:
